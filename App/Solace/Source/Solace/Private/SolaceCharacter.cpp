...
void ASolaceCharacter::ChooseTickets(bool bIsRanged)
{
	...
	if (bIsRanged)
	{
		if (WaitingRangedEnemies.Num() <= 0 && RangedTicketsUsed < MaxRangedTickets && TempSpot->IsValidLowLevel() && TempSpot->IsPendingKill() == false)
		{
	...
	{
		if (WaitingMeleeEnemies.Num() <= 0 && MeleeTicketsUsed < MaxMeleeTickets && TempSpot->IsValidLowLevel() && TempSpot->IsPendingKill() == false)
		{
			TempSpot->SetTicket(true);
			MeleeTicketsUsed++;
			return;
